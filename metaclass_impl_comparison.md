...
```c++
bitmask perms: unsigned {
  auto owner_read, owner_write, owner_exec, owner_all, ...;
};
```
...
```c++
bitmask directory_options: unsigned char {
  auto none, follow_directory_symlink, skip_permission_denied;
};
```
